...
def generate_lazyClass(classNumber, value, relation, classList, packageList, packageRelation):
...
#Only association
	for rel in relation:
		...
		elif str(rel[4])==str(classNumber) and rel[0]=='association':
				onlyRelation.append(rel)
...
#Delete Relations-> Priority: 1. Lowest Relations 2. Not the only relation for Packages
	...
	while i < int(round(numChange)): #not more then change
		...
		if onlyRelation[0][7]==1 and package!=[]:
			for j in classList:
				if str(onlyRelation[0][4])==str(j[2]) or str(onlyRelation[0][3])==str(j[2]):
					ID2=j[0]
					break
